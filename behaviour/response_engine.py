...
class ResponseEngine:
    ...
    def decide(self, behaviour: str, attack_type: str, confidence: float) -> Dict[str, Any]:
        ...
        # 1. Sticky Sandbox + Threat State Override
        # If the attacker is already deeply tracked as a severe threat, bypass 
        # single-event low-confidence fallback. They must remain CONTAINED.
        if behaviour in ("MALICIOUS", "CONFIRMED_ATTACK", "KILL_CHAIN_CONFIRMED", "PERSISTENT_ATTACKER", "APT"):
            return self._contain(f"Sticky Sandbox enforced (overriding confidence) for {behaviour}")

        # 2. Low Confidence Fallback for new/benign actors
        if confidence < 0.4:
            return self._allow("low confidence activity")

        # FIX #7: Handle kill chain state is now handled by the Sticky Sandbox above
        # (It will return CONTAIN, preserving the trap logic).
