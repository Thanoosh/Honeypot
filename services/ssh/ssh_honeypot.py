...
# Commands in this set MUST NEVER trigger an AI/LLM call.
# They are always handled locally by FakeShell with zero added latency.
# This is the strict enforcement boundary between the deterministic and AI layers.
# ─────────────────────────────────────────────

DETERMINISTIC_COMMANDS = {
    # Navigation — filesystem state is always local truth
    "cd", "pwd", "ls",
    # Identity — never changes within a session
    "whoami", "id", "hostname",
    # File reading — FAKE_FILE_CONTENTS is canonical; AI must NEVER contradict it
    "cat", "find",
    # System commands — all have static/seeded local handlers
    "uname", "uptime", "ps", "sudo", "su", "chmod", "chown",
    # Network — static fake output
    "netstat", "ss", "ifconfig", "ip", "wget", "curl", "ssh", "scp",
    # Language/Text
    "python", "python3", "nano", "vi", "vim",
    # Packages/System/Containers
...
def send_event(event_type: str, details: dict, high_value: bool = False):
    ...
    try:
        ...
        res = requests.post(
            CORE_EVENT_ENDPOINT,
            json=payload,
            timeout=4,   # 4s: generous margin above the 3s LLM deadline in core/main.py
...
class FakeShell:
    ...
    def __init__(self, username: str, client_ip: str, channel):
        ...
        # Seed a session-local RNG using the client IP and a stable boot hash.
        # This ensures file sizes, system load, and other "random" metadata are
        # identical across repeated commands from the same attacker session,
        # preventing observable variation that reveals the honeypot.
        _boot_seed = int(hashlib.md5(client_ip.encode()).hexdigest()[:8], 16)
        self._rng = random.Random(_boot_seed)
    ...
    def handle(self, cmd: str) -> bool:
        ...
        # Shell metacharacters imply compound exploit payloads that local handlers can't fully simulate.
        # Unknown commands or compound commands fall through to the AI / Slow Path.
        has_metachars = any(c in cmd for c in ("|", "&&", ";", "$(", ">", "<"))
        is_deterministic = base in DETERMINISTIC_COMMANDS and not has_metachars

        # ─────────────────────────────────────────────
        # EVENT LOGGING + AI CONTAIN ROUTING
        # Deterministic commands: fire-and-forget async (zero blocking latency).
        # Non-deterministic commands: synchronous — we need the CONTAIN response.
        # ─────────────────────────────────────────────
        event_payload = {
            "command": cmd,
            "client_ip": self.client_ip,
            ...
            "available_files": FAKE_FS.get(self.cwd, []),
            "command_number": self.command_count,
        }

        api_response = None
        if is_deterministic:
            # Fire-and-forget: log the event without blocking command execution
            threading.Thread(
                target=send_event,
                args=("SSH_COMMAND", event_payload),
                daemon=True,
            ).start()
        else:
            # Synchronous: we need the response to decide CONTAIN vs local handler
            api_response = send_event("SSH_COMMAND", event_payload)

            # ─────────────────────────────────────────────
            # AI CONTAIN OVERRIDE (non-deterministic commands only)
            # ─────────────────────────────────────────────
            if (
                api_response
                and api_response.get("response", {}).get("action") == "CONTAIN"
                and api_response.get("terminal_output")
            ):
                for line in api_response["terminal_output"].split("\n"):
                    self._out(line)
                return True
        ...
        # All commands below always run locally with zero AI calls.
        # ─────────────────────────────────────────────
        ...
        if base in ("exit", "logout", "quit"):
            ...
            if "-la" in args or "-al" in args or "-l" in args:
                # Use the session-local seeded RNG for stable fake metadata
                self._out(f"total {self._rng.randint(20, 80)}")
                self._out(f"drwxr-xr-x  {self._rng.randint(2,8)} {self.username} {self.username}  4096 Nov  3 08:12 .")
                self._out(f"drwxr-xr-x  {self._rng.randint(3,6)} {self.username} {self.username}  4096 Oct 28 14:30 ..")
                for item in contents:
                    is_dir = self._resolve_path(f"{target}/{item}") in FAKE_FS
                    perm = "drwxr-xr-x" if is_dir else "-rw-r--r--"
                    size = self._rng.randint(128, 8192) if not is_dir else 4096
                    month = self._rng.choice(["Sep", "Oct", "Nov"])
            ...
            uid = 0 if self.username == "root" else self._rng.randint(1000, 1003)
            ...
            days = self._rng.randint(10, 120)
            self._out(f" 14:32:01 up {days} days,  2:14,  1 user,  load average: 0.{self._rng.randint(10,40)}, 0.{self._rng.randint(10,40)}, 0.{self._rng.randint(10,40)}")
            ...
            self._out(f"{self._rng.randint(1000,2000)}   pts/0    00:00:00 bash")
            self._out(f"{self._rng.randint(2001,3000)}   pts/0    00:00:00 ps")
...
def handle_client(client, addr):
    ...
    try:
        ...
        # Seed MOTD randomness per client IP — stable for the same attacker.
        # Without seeding, system load/disk % changed on every reconnect,
        # creating an observable fingerprint of the honeypot.
        _motd_rng = random.Random(int(hashlib.md5(ip.encode()).hexdigest()[:8], 16))
        motd = MOTD.format(
            date=time.strftime("%a %b %d %H:%M:%S UTC %Y"),
            load=_motd_rng.randint(10, 60),
            disk=_motd_rng.randint(30, 75),
            mem=_motd_rng.randint(20, 60),
            last_login=time.strftime(
                "%a %b %d %H:%M:%S %Y",
                time.localtime(time.time() - _motd_rng.randint(3600, 86400))
            ),
            last_ip=f"192.168.1.{_motd_rng.randint(2, 254)}",
        )
