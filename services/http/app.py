...
# Per-IP attacker state — isolates deception mode per session.
# Key: client IP string. Value: dict with 'mode' key.
# Prevents cross-contamination between concurrent attacker sessions
# and fixes the race condition on the global SERVICE_MODE.
ATTACKER_STATE: dict = {}
...
def _get_attacker_state(ip: str) -> dict:
    """Return or initialise per-IP state dictionary. Thread-safe via GIL on dict ops."""
    if ip not in ATTACKER_STATE:
        ATTACKER_STATE[ip] = {"mode": "NORMAL"}
    return ATTACKER_STATE[ip]
...
def send_event(event_type: str, details: dict, mitre_id: str = "", mitre_name: str = "") -> dict:
    ...
    State is now scoped per client IP — no global mutation, no race conditions.
    """
    ip = get_client_ip()
    state = _get_attacker_state(ip)
    try:
        payload = {
        ...
        if resp.ok:
            ...
            # Update state scoped to this IP only — never pollutes other sessions
            new_mode = response.get("service_mode")
            if new_mode:
                state["mode"] = new_mode
...
def login():
    ...
    if request.method == "POST":
        ...
        if detect_sqli(combined):
            ...
            # Read mode from THIS attacker's per-IP state — not a global.
            attacker_mode = _get_attacker_state(get_client_ip())["mode"]
            if attacker_mode in ("DECEPTION", "FAKE_DB"):
